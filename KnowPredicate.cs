...
namespace CPOR_MA
{
    class KnowPredicate : Predicate
    {
        ...
        public KnowPredicate(Predicate p) : this(p, !p.Negation, false)
        {
            Console.Write("");
        }

        public KnowPredicate(Predicate p, Argument cAgent)  : base("K" + p.Name)
        {
            Predicate pWithAgent = null;
            if (p is ParametrizedPredicate)
            {
                pWithAgent = new ParametrizedPredicate((ParametrizedPredicate)p);
                ((ParametrizedPredicate)pWithAgent).AddParameter(cAgent);
            }
            if (p is GroundedPredicate)
            {
                pWithAgent = new GroundedPredicate((GroundedPredicate)p);
                ((GroundedPredicate)pWithAgent).AddConstant((Constant)cAgent);

            }
            Knowledge = pWithAgent;
            if (p.Negation)
            {
                Value = false;
            }
        ...
        public KnowPredicate(Predicate p, bool bValue, bool bParametrized) : base("N/A")
        {
            /*
            if (bValue)
                Name = "K" + p.Name;
