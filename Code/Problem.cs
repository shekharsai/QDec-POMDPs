...
namespace CPOR_MA
{
    public class Problem
    {
        ...
        // Shashank's code
        public List<CompoundFormula> hiddenFormula()
        {
            return m_lHidden;
        }
        public HashSet<Predicate> knownPredicates()
        {
            return m_lKnown;
        }
        public String name()
                                        ...
                                        List<Predicate> lTrueState, Dictionary<string, double> dFunctionValues, bool bOnlyIdentifyStates)
        {
            ...
            foreach (GroundedPredicate gp in lObserved)
            {
                ...
                if (!Domain.AlwaysKnown(gp) || (SDRPlanner.MultiAgent && !Domain.AlwaysConstant(gp)))//in MA we do not know which actions are executed, and hence only alwaysknown + alwaysconstant applies
                {
                    if (SDRPlanner.MultiAgent)
                    {
                        foreach (Constant cAgent in Domain.Agents)
                        {
                            string sAgentKP = sKP;
                            sAgentKP += " " + cAgent.Name;
                            if (gp.Negation)
                                sAgentKP += " " + Domain.FALSE_VALUE;
                            else
                                sAgentKP += " " + Domain.TRUE_VALUE;
                            sw.WriteLine(sAgentKP + ")");
                        }
                    }
                    else
                    {
            ...
            HashSet<GroundedPredicate> lFalseInitFacts = Domain.ComputeFalseInitialFacts(dTags);
            foreach (GroundedPredicate gp in lFalseInitFacts)
            {
                sKP = "(K" + gp.Name;
                sP = "(" + gp.Name;
                foreach (Constant c in gp.Constants)
            ...
            foreach (KeyValuePair<string, List<Predicate>> p in dTags)
            {
                ...
                foreach (GroundedPredicate gp in p.Value)
                {
                    ...
                    if (SDRPlanner.MultiAgent)
                    {
                        foreach (Constant cAgent in Domain.Agents)
                        {
                            sKP = GenerateKnowGivenLine(gp, p.Key, false, cAgent);
                            sw.WriteLine(sKP);
                        }
                    }
                    else
            ...
            foreach (Predicate p in lGoalPredicates)
            {
                if (!Domain.AlwaysKnown(p))
                {
                    ...
                    {
                        foreach (Constant cAgent in Domain.Agents)
                            cfGoal.AddOperand(new KnowPredicate(p, cAgent));
        ...
        private string GenerateKnowGivenLine(GroundedPredicate gp, string sTag, bool bKnowWhether, Constant cAgent)
        {
            string sKP = "";
            if (bKnowWhether)
                sKP = "(KWGiven" + gp.Name;
        ...
        public MemoryStream WriteTaggedProblem(Dictionary<string, List<Predicate>> dTags, IEnumerable<Predicate> lObserved,
                                        List<Predicate> lTrueState, Dictionary<string, double> dFunctionValues, bool bOnlyIdentifyStates,
                                        Dictionary<Constant,HashSet<Predicate>> dAgentObserved)
        {
            MemoryStream msProblem = new MemoryStream();
            StreamWriter sw = new StreamWriter(msProblem);
            sw.WriteLine("(define (problem K" + Name + ")");
            ...
            //sw.WriteLine("(next-cost-counter ZERO ONE)");
            //sw.WriteLine("(next-cost-counter ONE TWO)");
            //sw.WriteLine("(next-cost-counter TWO THREE)");
            //sw.WriteLine("(next-cost-counter THREE FOUR)");
            ...
            foreach (GroundedPredicate gp in lObserved)
            {
                ...
                if (!Domain.AlwaysKnown(gp) || (SDRPlanner.MultiAgent && !Domain.AlwaysConstant(gp))) // in MA we do not know which actions are executed, and hence only alwaysknown + alwaysconstant applies
                {
                    if (SDRPlanner.MultiAgent)
                    {
                        bool isDuplicate = false; // shashank: might be needed below
                        foreach (GroundedPredicate gpLocal in lFalseInitFacts)
                        {
                            if (gp.Name.ToLower().Equals(gpLocal.Name.ToLower()) && gp.Constants.Count == gpLocal.Constants.Count)
                                for (int i = 0; i < gp.Constants.Count; i++)
                                    if (gp.Constants[i].ToString().ToLower().Equals(gpLocal.Constants[i].ToString().ToLower()))
                                        isDuplicate = true;
                        }

                        // shashank
                        // the check below is necessary to avoid repeating propositions but as I saw in Rovers R1, it makes sometimes
                        // the observed predicate hidden in the initial state
            ...
            if(SDRPlanner.MultiAgent)
            {   
                // writing agent specific knowledge
                foreach(Constant cAgent in dAgentObserved.Keys)
                {
                    foreach(GroundedPredicate gp in dAgentObserved[cAgent])
                    {
                        string sAgentKP = "(K" + gp.Name;
                        foreach (Constant c in gp.Constants)
                        {
                            sAgentKP += " " + c.Name;
                        }
            ...
            // shashank: moving the below line a bit upward, there are some propostions repeated.
            // HashSet<GroundedPredicate> lFalseInitFacts = Domain.ComputeFalseInitialFacts(dTags);
            foreach (GroundedPredicate gp in lFalseInitFacts)
            {
                sKP = "(K" + gp.Name;
            ...
            foreach (Predicate p in lGoalPredicates)
            {
                if (!Domain.AlwaysKnown(p))
                {
                    if (SDRPlanner.MultiAgent)
                    {
                        ...
                        //not sure that we should force all agents to know that the goal was reached.
                        foreach (Constant cAgent in Domain.Agents)
                            cfOr.AddOperand(new KnowPredicate(p, cAgent));
                        //cfGoal.AddOperand(new KnowPredicate(p));
                        cfGoal.AddOperand(cfOr);
                    }
                    else
                    {
                        cfGoal.AddOperand(new KnowPredicate(p));
