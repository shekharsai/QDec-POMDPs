...
namespace CPOR_MA
{
    public class Action
    {
        ...
        private Formula m_fPreconditions;
        public Formula Preconditions {
            get
            {
                return m_fPreconditions;
            }
            set
            {
                m_fPreconditions = value;
                if(m_fPreconditions != null)
                {
                    if (SDRPlanner.MultiAgent)
                    {
                        //this is a hack - the agents executing the action must be in the preconditions
                        //it would be cleaner for each action to declare explicitly which agents execute it
                        ExecutingAgents = new HashSet<Constant>();
                        HashSet<Constant> hsConstants = m_fPreconditions.GetConstants();
                        foreach (Constant c in hsConstants)
                            if (c.Type.ToLower() == Domain.AGENT.ToLower())
                                ExecutingAgents.Add(c);
                    }
                }
            }
        ...
        public Dictionary<Argument, Formula> ObserveAgent { get; set; }
        public HashSet<Constant> ExecutingAgents { private set; get; }
        ...
        public Action(string sName)
        {
            ...
            ObserveAgent = new Dictionary<Argument, Formula>();
            ExecutingAgents = new HashSet<Constant>();
        }
        ...
        public Action NonConditionalObservationTranslation(Dictionary<string, List<Predicate>> dTags, List<string> lAlwaysKnown, Domain d, bool bTrue)
        {
            Action aNew = Clone();
            if (bTrue)
                aNew.Name += "-T";
            ...
            List<Argument> lAgents = new List<Argument>();
            if (this is ParametrizedAction)
            {
                foreach (Parameter p in ((ParametrizedAction)this).Parameters)
                {
                    ...
                    {
                        if (p.Type == "agent")
                            lAgents.Add((Argument)p);
                    }

                }
            }

            CompoundFormula cfPreconditions = GetKnowledgePreconditions(lAgents, d);
            HashSet<Predicate> lKnowPreconditions = new HashSet<Predicate>();
            if (Observe == null)
                throw new NotImplementedException();
            CompoundFormula cfEffects = new CompoundFormula("and");
            ...
            if (Effects != null)
            {
                ...
                // We allow actions that have effects and obervations only if the effects are over always known predicates.
                foreach (Predicate p in lKnowEffects)
                {
                    if(!d.AlwaysKnown(p))
                        throw new Exception("You can only have known effects with observations");
                }
            }
            ...
            // Allow observations only over something that is not already known
            if (SDRPlanner.MultiAgent)
            {                
                if (!Name.Contains("phase")) 
                {
                    foreach (Argument cAgent in lAgents)
                    {
                        cfPreconditions.AddOperand(new PredicateFormula(new KnowPredicate(pObserve, cAgent).Negate()));
                        cfPreconditions.AddOperand(new PredicateFormula(new KnowPredicate(pObserve.Negate(), cAgent).Negate()));
                    }
                }                
            }
            else
            {
                cfPreconditions.AddOperand(new PredicateFormula(new KnowPredicate(pObserve).Negate()));
                cfPreconditions.AddOperand(new PredicateFormula(new KnowPredicate(pObserve.Negate()).Negate()));
            }
            ...
            if (SDRPlanner.MultiAgent)
            {
                ...
                {
                    if (bTrue)
                        cfEffects.AddOperand(new PredicateFormula(new KnowPredicate(pObserve, cAgent)));
                    else
                        cfEffects.AddOperand(new PredicateFormula(new KnowPredicate(pObserve.Negate(), cAgent)));
                    if (SDRPlanner.MultiAgent && SDRPlanner.ForceObservationBeforeReasoning)
                    {
                        ...
                        ppObserved.AddParameter(cAgent);
                        cfEffects.AddOperand(ppObserved);
                    }
                }
            }
            else
            {
                if (bTrue)
                    cfEffects.AddOperand(new PredicateFormula(new KnowPredicate(pObserve)));
                else
                    cfEffects.AddOperand(new PredicateFormula(new KnowPredicate(pObserve.Negate())));
            }
        ...
        public CompoundFormula GetKnowledgePreconditions(List<Argument> lAgents, Domain d)
        {
            CompoundFormula cfPreconditions = new CompoundFormula("and");
            HashSet<Predicate> lKnowPreconditions = new HashSet<Predicate>();
            if (Preconditions != null)
            {
                ...
                foreach (Predicate p in lKnowPreconditions)
                {
                    if (!d.AlwaysKnown(p))
                    {
                        if (SDRPlanner.MultiAgent)
                        {
                            foreach (Argument aAgent in lAgents)
                                cfPreconditions.AddOperand(new PredicateFormula(new KnowPredicate(p, aAgent)));
                        }
                        else
                            cfPreconditions.AddOperand(new PredicateFormula(new KnowPredicate(p)));
                    }
        ...
        // shashank: to handle signaling
        public List<Predicate> KnowPredicatesFromEffect(List<Argument> lAgents, Formula eff, Domain d)
        {
            List<Predicate> lKnowPredicates = new List<Predicate>();
            List<Predicate> lPredSignalOn = new List<Predicate> ();
            if (Effects is CompoundFormula)
            {
                CompoundFormula cfEffects = (CompoundFormula)Effects;
                if (cfEffects.Operator == "when" && cfEffects.Operands[0] is PredicateFormula)
                {
                    lPredSignalOn.Add(((PredicateFormula) cfEffects.Operands[0]).Predicate);                                            
                }
                else if(cfEffects.Operator == "and")
                {
                    List<Formula> lCompundFormula = cfEffects.GetAllOperands();
                    foreach (Formula f in lCompundFormula)
                    {
                        if (f is CompoundFormula)
                            if (((CompoundFormula) f).Operator == "when" && ((CompoundFormula) f).Operands[0] is PredicateFormula)
                                lPredSignalOn.Add(((PredicateFormula)((CompoundFormula) f).Operands[0]).Predicate);
                    }
                }
                else
                    throw new Exception("Handle (and) properly as in p->q, only p should be ~Kp^~K~p");
            }

            foreach (Predicate pSignalOn in lPredSignalOn)
            {
                if (!d.AlwaysKnown(pSignalOn))
                {
                    foreach (Argument aAgent in lAgents)
                    {
                        lKnowPredicates.Add(new KnowPredicate(pSignalOn, aAgent).Negate());
                        lKnowPredicates.Add(new KnowPredicate(pSignalOn.Negate(), aAgent).Negate());
                    }
                }
            }
            return lKnowPredicates;
        }

        public Action AddTaggedConditions(Dictionary<string, List<Predicate>> dTags, Domain d)
        {
            Action aNew = Clone();
            List<CompoundFormula> lConditions = new List<CompoundFormula>();
            List<Formula> lObligatory = new List<Formula>();
            ...
            // to handle signaling, not sure how to identify a signaling action
            List<Predicate> lKnowPredicates = new List<Predicate>();
            if (Name.ToLower().Contains("signaling-") || Name.ToLower().Contains("macro-"))
                lKnowPredicates = KnowPredicatesFromEffect(lAgents, Effects, d);

            foreach (KnowPredicate p in lKnowPredicates)
            ...
            if (Effects != null)
            {
                ...
                foreach (Predicate p in lKnowEffects)
                {
                    if (!d.AlwaysKnown(p))
                    {
                        Predicate pKEffect = null;
                        if (SDRPlanner.MultiAgent)
                        {
                            foreach (Argument aAgent in lAgents)
                            {
                                pKEffect = new KnowPredicate(p, aAgent);
                                cfEffects.AddOperand(pKEffect);
                                pKEffect = new KnowPredicate(p.Negate(), aAgent);
                                cfEffects.AddOperand(pKEffect.Negate());
                            }
                        }
                        else
                        {
                            pKEffect = new KnowPredicate(p);
                            cfEffects.AddOperand(pKEffect);
                            pKEffect = new KnowPredicate(p.Negate());
                            cfEffects.AddOperand(pKEffect.Negate());
                        }
                        ...
                        foreach (string sTag in dTags.Keys)
                        {
                            if (SDRPlanner.MultiAgent)
                            {
                                foreach (Argument aAgent in lAgents)
                                {
                                    pKEffect = p.GenerateKnowGiven(sTag, aAgent);
                                    cfEffects.AddOperand(pKEffect);
                                    pKEffect = p.Negate().GenerateKnowGiven(sTag, aAgent);
                                    cfEffects.AddOperand(pKEffect.Negate());
                                }
                            }
                            else
                ...
                foreach (CompoundFormula cfCondition in lConditions)
                {
                    ...
                    if (SDRPlanner.MultiAgent)
                    {
                        // Need to think what happens here - all agents gain and forget or only the agent applying the action?
                        // Maybe all agent lose knowledge and only the agent acting gains knowledge?
                        // foreach (Constant cAgent in d.Agents)
                        foreach (Argument aAgent in lAgents)
                        {
                            CompoundFormula cfK = CreateKnowledgeGainCondition(cfCondition, d.m_lAlwaysKnown, aAgent);
                            if (cfK != null)
                                cfEffects.AddOperand(cfK);
                            cfK = CreateKnowledgeLossCondition(cfCondition, d.m_lAlwaysKnown, aAgent);
                            if (cfK != null)
                                cfEffects.AddOperand(cfK);
                            foreach (string sTag in dTags.Keys)
                            {
                                cfK = CreateTaggedKnowledgeGainCondition(cfCondition, sTag, d.m_lAlwaysKnown, false, aAgent);
                                if (cfK != null)
                                    cfEffects.AddOperand(cfK);
                                cfK = CreateTaggedKnowledgeLossCondition(cfCondition, sTag, d.m_lAlwaysKnown, aAgent);
                                if (cfK != null)
                                    cfEffects.AddOperand(cfK);
                            }
                        }
                    ...
                    else
                    {
                        ...
                        if (cfK != null)
                            cfEffects.AddOperand(cfK);
                        cfK = CreateKnowledgeLossCondition(cfCondition, d.m_lAlwaysKnown);
                        if (cfK != null)
                            cfEffects.AddOperand(cfK);
                        foreach (string sTag in dTags.Keys)
                        {
                            cfK = CreateTaggedKnowledgeGainCondition(cfCondition, sTag, d.m_lAlwaysKnown, false);
                            if (cfK != null)
                                cfEffects.AddOperand(cfK);
                            cfK = CreateTaggedKnowledgeLossCondition(cfCondition, sTag, d.m_lAlwaysKnown);
                            if (cfK != null)
                                cfEffects.AddOperand(cfK);
                        }
                    }
            ...
            if (Observe != null)
            {
                ...
                cfWhen.AddOperand(new KnowPredicate(pObserve.Negate()));
                ((CompoundFormula)aNew.Effects).AddOperand(cfWhen);
                //need to add !Kp to preconditions

            }
            if (SDRPlanner.MultiAgent && ObserveAgent.Count != 0)
            {
                if (aNew.Effects == null)
                    aNew.Effects = new CompoundFormula("and");

                foreach (Argument cAgent in ObserveAgent.Keys)
                {
                    HashSet<Predicate> lObserve = ObserveAgent[cAgent].GetAllPredicates();
                    foreach (Predicate pObserve in lObserve)
                    {
                        CompoundFormula cfWhen = new CompoundFormula("when");
                        CompoundFormula cfAnd = new CompoundFormula("and");
                        Predicate pK = new KnowPredicate(pObserve, cAgent);
                        Predicate pNK = new KnowPredicate(pObserve.Negate(), cAgent);
                        cfWhen.AddOperand(pObserve);
                        cfAnd.AddOperand(pK);
                        //cfAnd.AddOperand(new KnowPredicate(pObserve, new Constant(Domain.AGENT, "ALL")));//not sure what this is for
                        ...
                        cfWhen.AddOperand(pObserve.Negate());
                        cfAnd = new CompoundFormula("and");
                        cfAnd.AddOperand(pNK);
                        //cfAnd.AddOperand(new KnowPredicate(pObserve.Negate(), new Constant(Domain.AGENT, "ALL")));//not sure what this is for
                        cfWhen.AddOperand(cfAnd);

                        ((CompoundFormula)aNew.Effects).AddOperand(cfWhen);

                        cfPreconditions.AddOperand(pK.Negate());
                        cfPreconditions.AddOperand(pNK.Negate());
        ...
        */
        ...
        private CompoundFormula CreateKnowledgeGainCondition(CompoundFormula cfCondition, List<string> lAlwaysKnown, Argument cAgent)
        {
            CompoundFormula cfWhen = new CompoundFormula("when");
            ...
            Formula fPreconditions = cfCondition.Operands[0].GetKnowledgeFormula(lAlwaysKnown, cAgent);
            CompoundFormula cfEffects = new CompoundFormula("and");
            foreach (Predicate p in lEffects)
            {
                cfEffects.AddOperand(new KnowPredicate(p, cAgent));
                cfEffects.AddOperand(new KnowPredicate(p.Negate(), cAgent).Negate());
        ...
        private CompoundFormula CreateTaggedKnowledgeGainCondition(CompoundFormula cfCondition, string sTag, List<string> lAlwaysKnown, bool bNonDetEffect, Argument cAgent)
        {
            CompoundFormula cfWhen = new CompoundFormula("when");
            HashSet<Predicate> lPreconditions = new HashSet<Predicate>();
            HashSet<Predicate> lEffects = new HashSet<Predicate>();
            cfCondition.Operands[0].GetAllPredicates(lPreconditions);
            cfCondition.Operands[1].GetAllPredicates(lEffects);
            Formula cfPreconditions = cfCondition.Operands[0].GenerateGiven(sTag, lAlwaysKnown, cAgent);
            CompoundFormula cfEffects = new CompoundFormula("and");
            foreach (Predicate p in lEffects)
            {
                if (lAlwaysKnown == null || !lAlwaysKnown.Contains(p.Name))
                {
                    Predicate pKEffect = p.GenerateKnowGiven(sTag, cAgent);
                    cfEffects.AddOperand(pKEffect);
                    if (bNonDetEffect)
                    {
                        throw new NotImplementedException();
        ...
        private CompoundFormula CreateTaggedKnowledgeLossCondition(CompoundFormula cfCondition, string sTag, List<string> lAlwaysKnown, Argument cAgent)
        {
            CompoundFormula cfWhen = new CompoundFormula("when");
            HashSet<Predicate> lPreconditions = new HashSet<Predicate>();
            HashSet<Predicate> lEffects = new HashSet<Predicate>();
            ...
            foreach (Predicate p in lPreconditions)
            {
                if (lAlwaysKnown == null || !lAlwaysKnown.Contains(p.Name))
                {
                    Predicate pKGiven = p.Negate().GenerateKnowGiven(sTag, cAgent);
        ...
        private CompoundFormula CreateKnowledgeLossCondition(CompoundFormula cfCondition, List<string> lAlwaysKnown, Argument cAgent)
        {
            CompoundFormula cfWhen = new CompoundFormula("when");
            HashSet<Predicate> lPreconditions = new HashSet<Predicate>();
            HashSet<Predicate> lEffects = new HashSet<Predicate>();
            ...
            foreach (Predicate p in lPreconditions)
            {
                ...
                if (lAlwaysKnown == null || !lAlwaysKnown.Contains(p.Name))
                {
                    cfPreconditions.AddOperand(new KnowPredicate(p.Negate(), cAgent).Negate());
                }
            }
            //if (bAllKnown)//if all given are known then there is no knowledge loss
            //    return null;
            CompoundFormula cfEffects = new CompoundFormula("and");
            foreach (Predicate p in lEffects)
            {
