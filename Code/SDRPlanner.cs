...
namespace CPOR_MA
{
    class SDRPlanner
    {
        ...
        public static bool UseFilesForPlanners = true; //shashank: needs for the experiments done in the Rovers domain
        ...
        // TODO - to remove 
        public static string sRoverPath = null;

        public SDRPlanner(string sPath, Domain d, Problem p, bool multiAgentTeamPlan)
        {
            sRoverPath = sPath;
            Data = new ExecutionData(sPath, d, p, multiAgentTeamPlan);
        }
        ...
        private List<string> RunPlannerNoFiles(MemoryStream msModels, int iIndex)
        {
            ...
            // p.StartInfo.FileName = Program.BASE_PATH + @"\Planners\ff-large.exe";
            ...
            /**
            StreamWriter sw = new StreamWriter(Program.BASE_PATH + @"\temp\tmp_models.txt");
            msModels.Position = 0;
            ...
            **/
            ...
            /**
            msModels.Position = 0;
            StreamWriter swTest = new StreamWriter(Program.BASE_PATH + @"\temp\test.pddl");
            ...
            **/
            ...
            if (!p.WaitForExit((int)PlannerTimeout.TotalMilliseconds)) // two minutes max
            {
                p.Kill();
                return null;
            }
        ...
        private List<string> RunPlannerNoFiles_try(MemoryStream msModels, int iIndex)
        {
            if (Planner != Planners.FF)
            {
                throw new NotImplementedException();
            ...
            //see how much time metric-ff takes
            ...
            StreamWriter sw = new StreamWriter(Program.BASE_PATH + @"\temp\dom.pddl");
            msModels.Position = 0;
            StreamReader sr = new StreamReader(msModels);
            sw.Write(sr.ReadToEnd());
            sw.Close();
            ...
            msModels.Position = 0;
            StreamWriter swTest = new StreamWriter(Program.BASE_PATH + @"\temp\prob.pddl");
            StreamReader srTest = new StreamReader(msModels);
            swTest.Write(srTest.ReadToEnd());
            swTest.Close();
        ...
        private List<string> RunPlannerNoFilesForMetricPlanning(MemoryStream msModels, int iIndex) 
        {
            ...
            FFProcesses[Thread.CurrentThread] = p;
            p.StartInfo.WorkingDirectory = Program.BASE_PATH + @"\temp\";
        ...
        public List<string> RunPlanner(string sPath, MemoryStream msModel, int iIndex)
        {
            ...
            {
                Console.WriteLine("(SDRPlanner.cs) (RunPlanner call) Time Out!!");
        ...
        private List<string> Plan (string sPath, PartiallySpecifiedState pssCurrent, out State sChosen, ref List<Action> cPlan)
        {
            ...
            **/
            ...
            while (!bDone)
            {
                ...
                {
                    foreach (FileInfo fi in di.GetFiles())
                        if (((fi.Name.StartsWith("Kd") || fi.Name.StartsWith("Kp")) && fi.Name.EndsWith(".pddl")) || fi.Name.EndsWith(".pddl.ff"))
                            fi.Delete();
                    bDone = true;
                }
                catch (IOException e)
            ...
            if (ManualPlanning)
            {
                ...
                // it seems that (affected ?x) applies to all devices on their way to the device, 
                // and thus we can distinguish between them
                ...
                // List<Action> gAllAct = dK.GroundAllActions(pK);
        ...
        public StageTwoPlanPlusProblem OfflinePlanning(string sPath, Domain domain, Problem problem)
        {
            ...
            int cFailureCounter = 0;

            // for signaling we explicitly set timeout 3 seconds
            Stopwatch specialForSignaling = new Stopwatch();
            specialForSignaling.Start();
            bool bDone_Signaling = false;

            if (MultiAgent)
                Console.WriteLine("(Signaling) new call to CPOR is made for Team Planning!");

            while (!bDone && !bDone_Signaling)
            {                
                if ((specialForSignaling.Elapsed.Seconds + specialForSignaling.Elapsed.Minutes * 60) >= timeOut)
                {
                    Console.WriteLine("(SDRPlanner.cs) Time Out!!");
                    Console.WriteLine("The time limit is " + timeOut);
                    return new StageTwoPlanPlusProblem(null, null, false, true);
                }
                ...
                while (lPlan == null || lPlan.Count == 0)
                {
                    ...
                    else if (pssCurrent.IsGoalState())
                    {
                        pssCurrent.UpdateClosedStates(lClosedStates, alreadyVisitedStates, domain);
                    ...
                    else
                    {
                        ...
                        if (lPlan == null)
                        {
                            ...
                            */
                            ...
                            // this loop is only required when we use file for FF in rovers' domain.
                            // currently larfe-ff.exe has some issues in producing the plan in each attempt in Windows 10 cmd
                            while (count1 < 0)
                            {
                                if (lPlan == null)
                                {
                            ...
                            if (cFailureCounter > 2)
                            {                                
                                bDone_Signaling = true;
                                break;
                            }
                ...
                if (lPlan != null)
                {
                    ...
                    { 
                        ...
                        if (!IsReasoningAction(sAction.ToLower()))
                        {
                            ...
                            Action appendToPlanAction = null;
                            ...
                            pssCurrent.ApplyOffline(sAction, out appendToPlanAction, out fObserved, out psTrueState, out psFalseState);
                            // shashank
            ...
            // for agent based search at low level: shashank 
            if (cFailureCounter <= 2) // signaling
            {
                ...
                */
                ...
                // shashank
                StageTwoPlanPlusProblem stageTwoPlanPlus = new StageTwoPlanPlusProblem(problem, pssInitial.Plan, true, false);
                return stageTwoPlanPlus;
            }

            // shashank
            StageTwoPlanPlusProblem stageTwoPlanPluProblem = new StageTwoPlanPlusProblem(problem, null, false, false);
        ...
        public class ExecutionData
        {
            ...
            public ExecutionData(string sPath, Domain d, Problem p, bool multiAgentTeamPlan)
            {
                Domain = d;
                Problem = p;
                Path = sPath;
                Exception = "";
                MultiAgent = multiAgentTeamPlan;
            }
