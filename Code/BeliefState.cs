    ...
    public class BeliefState
    {
        ...
        */
        ...
        private List<State> ApplyActions(List<List<Predicate>> lChosen, List<Action> lActions)
        {
            ...
            {
                //Not maintaining the negative facts in the state. This may cause problems.
                //We did this because there was a negative precondition that was constant (not (can-observe ?a ?b)).
                //If this is problematic we can do a special case for negative constants when checkingfor preconditions.
                State s = new State(Problem, false);
        ...
        public State WriteTaggedDomainAndProblem(PartiallySpecifiedState pssCurrent, 
            string sDomainFile, string sProblemFile, List<Action> lAppliedActions, out int cTags, out MemoryStream msModels)
        {
            List<List<Predicate>> lChosen = ChooseStateSet();
            List<State> lStates = ApplyActions(lChosen, lAppliedActions);

            if(SDRPlanner.MultiAgent && lStates.Count == 1)
            {
                State sTag = new State(lStates[0]);
                foreach(Predicate p in lStates[0].Predicates)
                {
                    bool bAllObserved = true, bNoneOberved = true;
                    foreach (Constant cAgent in pssCurrent.AgentObserved.Keys)
                    {
                        if (pssCurrent.AgentObserved[cAgent].Contains(p) || pssCurrent.AgentObserved[cAgent].Contains(p.Negate()))
                            bNoneOberved = false;
                        else
                            bAllObserved = false;
                    }
                    if (!bNoneOberved && !bAllObserved)
                        sTag.AddPredicate(p.Negate());
                }
                lStates.Add(sTag);
            }
            ...
            if (lStates.Count == 1 && !Problem.Domain.ContainsNonDeterministicActions && SDRPlanner.Translation != SDRPlanner.Translations.BestCase)
            {
                ...
                msModels = new MemoryStream();
                StreamWriter sw = new StreamWriter(msModels);
                //StreamWriter domainFile = new StreamWriter(sDomainFile); //shashank
                msDomain.Position = 0;
                StreamReader srDomain = new StreamReader(msDomain);
                //StreamReader srDomain_copy = new StreamReader(msDomain); //shashank
                sw.Write(srDomain.ReadToEnd());
                //domainFile.Write(srDomain_copy.ReadToEnd()); //shashank
                sw.Write('\0');
                //domainFile.Close(); //shashank
                
                msProblem.Position = 0;
                StreamReader srProblem = new StreamReader(msProblem);
                //StreamReader srProblem_copy = new StreamReader(msProblem); //shashank
                //StreamWriter problemFile = new StreamWriter(sProblemFile); //shashank
                sw.Write(srProblem.ReadToEnd());
                //problemFile.Write(srProblem_copy.ReadToEnd()); //shashank
                sw.Write('\0');
                //problemFile.Close(); //shashank
                sw.Flush();
            ...
            // shashank: the below condition never appears to me
        ...
        {
            HashSet<Predicate> lObserved = new HashSet<Predicate>();
            Dictionary<string, List<Predicate>> dTags = null;
            if (SDRPlanner.MultiAgent)
                dTags = GetTags(lStates, pssCurrent.AgentObserved, lObserved);
            else
                dTags = GetTags(lStates, lObserved);
            ...
            {
                msDomain =
                    Problem.Domain.WriteKnowledgeDomain(Problem, pssCurrent.MishapCount, pssCurrent.MinMishapCount, pssCurrent.MishapType, false);
            }
            else if (SDRPlanner.Translation == SDRPlanner.Translations.SingleStateK)
            {
                msDomain =
                    Problem.Domain.WriteKnowledgeDomain(Problem, pssCurrent.MishapCount, pssCurrent.MinMishapCount, pssCurrent.MishapType, true);
            }
            ...
            if (SDRPlanner.Translation == SDRPlanner.Translations.BestCase || SDRPlanner.Translation == SDRPlanner.Translations.Conformant)
                ...
                msProblem = Problem.WriteTaggedProblem(dTags, lObserved, dTags.Values.First(), lStates.First().FunctionValues, bOnlyIdentifyStates, pssCurrent.AgentObserved); //the first tag is the real state
            else
                msProblem = Problem.WriteTaggedProblemNoState(dTags, lObserved, lStates.First().FunctionValues);
        ...
        private Dictionary<string, List<Predicate>> GetTags(List<State> lStates, Dictionary<Constant, HashSet<Predicate>> dObserved, HashSet<Predicate> lObserved)
        {
            Dictionary<string, List<Predicate>> dTags = new Dictionary<string, List<Predicate>>();
            int iTag = 0;
            //bugbug - what happens when there is only a single state?
            ...
            */
            HashSet<Predicate> lObservedAll = new HashSet<Predicate>();
            HashSet<Predicate> lObservedSome = new HashSet<Predicate>();

            foreach (Predicate p in lStates[0].Predicates)
            {
                bool bObservedAll = true;
                bool bObservedNone = true;
                foreach(Constant cAgent in dObserved.Keys)
                {
                    if (dObserved[cAgent].Contains(p))
                        bObservedNone = false;
                    else
                        bObservedAll = false;
                }
                if (bObservedAll)
                    lObservedAll.Add(p);
                else if (!bObservedNone)
                    lObservedSome.Add(p);
            }

            foreach(Predicate p in lObservedSome)
            {
                lStates[1].AddPredicate(p.Negate());
            }

            return GetTags(lStates, lObserved);
        }
