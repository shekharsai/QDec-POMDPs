...
namespace CPOR_MA
{
    class PredicateFormula : Formula
    {
        ...
        public override void GetAllOperands(List<Formula> lFormula)
        {
            /*
            Formula f = new Predicate();
            if (!lFormula.Contains((Formula) Predicate))
            {
                lFormula.Add((Predicate)Predicate);
            }
            */
        }
        ...
        public override Formula GenerateGiven(string sTag, List<string> lAlwaysKnown, Argument cAgent)
        {
            if (lAlwaysKnown.Contains(Predicate.Name))
                return this;
            PredicateFormula pfGiven = new PredicateFormula(Predicate.GenerateKnowGiven(sTag, cAgent));
            return pfGiven;
        }
        public override Formula GetKnowledgeFormula(List<string> lAlwaysKnown, Argument cAgent)
        {
            if (Predicate.Name == Domain.OPTION_PREDICATE)
                throw new NotImplementedException();//we never know an option value
            if (lAlwaysKnown.Contains(Predicate.Name))
                return this;
            return new PredicateFormula(new KnowPredicate(Predicate, cAgent));
        }
        ...
        {
            HashSet<Constant> hs = new HashSet<Constant>();
            if (Predicate is GroundedPredicate)
            {
                foreach (Constant c in ((GroundedPredicate)Predicate).Constants)
                    hs.Add(c);
            }
            return hs;
        }
