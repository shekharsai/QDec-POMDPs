...
(:predicates 
		...
		(communicated-soil-data ?dd - dummy); ?w - waypoint)
		(communicated-rock-data ?dd - dummy); ?w - waypoint)
		...
		(dummy-pred-needed ?dd - dummy) ;only to resolve some issue in the code, will fix it later
		(ach-paramet-goal ?agent1 - agent)
)
...
(:action observe-rock
	...
	:precondition (and (at ?x ?z) (can-sample-rock ?x))
...
(:action calibrate
	:parameters (?r - agent ?i - camera ?t - objective ?w - waypoint ?dd - dummy)
	:precondition (and 
...
(:action communicate-soil-data
 :parameters (?r - agent ?l - lander ;?p - waypoint  
              		?x - waypoint ?y - waypoint ?dd - dummy)
