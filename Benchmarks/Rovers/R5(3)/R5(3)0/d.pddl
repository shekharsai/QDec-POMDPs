...
(:predicates
	...
	(c-p-take-image-05b ?dd - dummy)
	(observe-take-image-05b ?dd - dummy)
)
(:action observe-vis-7xk
	:parameters ( ?x - agent ?t - objective ?z - waypoint )
	:precondition (and 
		(= ?x agent1)(= ?t objective1)(= ?z waypoint5)(observe-take-image-05b dd)
		(at ?x ?z)
	)
...
(:action observe-vis
	:parameters ( ?x - agent ?t - objective ?z - waypoint )
	:precondition (and 
		(or (not (= ?x agent1))(not (= ?t objective1))(not (= ?z waypoint5)))
		(at ?x ?z)
	)
	:observe (visible-from ?t ?z) 
)
...
(:action take-image
	...
	:precondition (and 
		(or (not (= ?r agent1))(not (= ?p waypoint5))(not (= ?o objective1))(not (= ?i camera1))(not (= ?m high-res)))
		(calibrated ?i ?r ?p ?o)
		(on-board ?i ?r)
		(equipped-for-imaging ?r)
		(supports ?i ?m)
...
(:action take-image-pe0j
	:parameters ( ?r - agent ?p - waypoint ?o - objective ?i - camera ?m - mode ?dd - dummy )
	:precondition (and 
		(= ?r agent1)(= ?p waypoint5)(= ?o objective1)(= ?i camera1)(= ?m high-res)(c-p-take-image-05b ?dd)
		(calibrated ?i ?r ?p ?o)
		(on-board ?i ?r)
		(equipped-for-imaging ?r)
		(supports ?i ?m)
	...
	:effect (and (when (c-p-take-image-05b ?dd) (not (c-p-take-image-05b ?dd)) )
		(have-image ?r ?o ?m)
		(not (calibrated ?i ?r ?p ?o)))
)
(:action commit-9k9c4-agent0-agent1-agent2-agent0-agent1-agent2
	:parameters (?a - agent)
	:precondition (and (not (observe-take-image-05b dd)) )
	:effect (and (c-p-take-image-05b dd))
)
(:action obs-pivv2-agent0-agent1-agent2-agent0-agent1-agent2
	:parameters (?a - agent)
	:precondition (and (not (observe-take-image-05b dd)))
	:effect (and (observe-take-image-05b dd))
)
