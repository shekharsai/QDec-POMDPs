...
:precondition 
(and 
	(or (not (= ?a1 agent0)) (not (= ?a2 agent1)) (not (= ?s a0store)) (not (= ?p waypoint4)))(at ?a1 ?p) (at ?a2 ?p) (not (same ?a1 ?a2)) (at-rock-sample ?p) (equipped-for-rock-analysis ?a1) (equipped-for-rock-analysis ?a2) (store-of ?s ?a1) (empty ?s ?a1))
:effect 
...
:precondition 
(and (= ?a1 agent0) (= ?a2 agent1) (= ?s a0store) (= ?p waypoint4) (c-p-sample-rock-kug ?dd) (at ?a1 ?p) (at ?a2 ?p) (not (same ?a1 ?a2)) (at-rock-sample ?p) (equipped-for-rock-analysis ?a1) (equipped-for-rock-analysis ?a2) (store-of ?s ?a1) (empty ?s ?a1))
:effect 
(and 
	(when (c-p-sample-rock-kug ?dd) (not (c-p-sample-rock-kug ?dd)))(not (empty ?s ?a1)) (full ?s ?a1) (have-rock-analysis ?a1) (not (at-rock-sample ?p)))
)

(:action commit-6nep6-agent0-agent1-agent2-agent3-agent0-agent1-agent2-agent3
:parameters ( ?a - agent)
:precondition (not (observe-sample-rock-kug dd))
:effect (c-p-sample-rock-kug dd)
)

(:action obs-jr0tl-agent0-agent1-agent2-agent3-agent0-agent1-agent2-agent3
:parameters ( ?a - agent)
:precondition (not (observe-sample-rock-kug dd))
:effect (observe-sample-rock-kug dd)
)
