...
(:predicates
	...
	(c-p-communicate-image-data-lgq ?dd - dummy)
	(observe-communicate-image-data-lgq ?dd - dummy)
)
(:action observe-vis-hah
	:parameters ( ?x - agent ?t - objective ?z - waypoint )
	:precondition (and 
		(= ?x agent0)(= ?t objective1)(= ?z waypoint4)(observe-communicate-image-data-lgq dd)
		(at ?x ?z)
	)
...
(:action observe-vis
	:parameters ( ?x - agent ?t - objective ?z - waypoint )
	:precondition (and 
		(or (not (= ?x agent0))(not (= ?t objective1))(not (= ?z waypoint4)))
		(at ?x ?z)
	)
	:observe (visible-from ?t ?z) 
)
...
(:action communicate-image-data
	...
	:precondition (and 
		(or (not (= ?r agent0))(not (= ?l general))(not (= ?o objective1))(not (= ?m high-res))(not (= ?x waypoint1))(not (= ?y waypoint0))(not (= ?dd dd)))
		(at ?r ?x)
		(at-lander ?l ?y)
		(have-image ?r ?o ?m)
		(visible ?x ?y)
...
(:action communicate-image-data-ixko
	:parameters ( ?r - agent ?l - lander ?o - objective ?m - mode ?x - waypoint ?y - waypoint ?dd - dummy )
	:precondition (and 
		(= ?r agent0)(= ?l general)(= ?o objective1)(= ?m high-res)(= ?x waypoint1)(= ?y waypoint0)(= ?dd dd)(c-p-communicate-image-data-lgq ?dd)
		(at ?r ?x)
		(at-lander ?l ?y)
		(have-image ?r ?o ?m)
		(visible ?x ?y)
	)
	:effect (and (when (c-p-communicate-image-data-lgq ?dd) (not (c-p-communicate-image-data-lgq ?dd)) )
		(communicated-image-data ?o ?m)
		(dummy-pred-needed ?dd))
)
(:action commit-lv2ch-agent0-agent1-agent2-agent3-agent0-agent1-agent2-agent3
	:parameters (?a - agent)
	:precondition (and (not (observe-communicate-image-data-lgq dd)) )
	:effect (and (c-p-communicate-image-data-lgq dd))
)
(:action obs-mafr1-agent0-agent1-agent2-agent3-agent0-agent1-agent2-agent3
	:parameters (?a - agent)
	:precondition (and (not (observe-communicate-image-data-lgq dd)))
	:effect (and (observe-communicate-image-data-lgq dd))
)
(:action sample-soil
