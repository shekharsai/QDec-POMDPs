...
namespace CPOR_MA
{
    public class PartiallySpecifiedState
    {
        ...
        public Dictionary<Constant, HashSet<Predicate>> AgentObserved { get { return m_dAgentObserved; } }
        ...
        protected Dictionary<Constant, HashSet<Predicate>> m_dAgentObserved;
        protected Dictionary<Constant, HashSet<Predicate>> m_dAgentHidden;
        ...
        public Formula GeneratingObservation
        {
            ...
            private set
            {
                ...
                if (m_fObservation != null)
                {
                    ...
                    {
                        foreach(Constant cAgent in GeneratingAction.ExecutingAgents)
                        {
                            foreach (GroundedPredicate gp in m_fObservation.GetAllPredicates())
                            {
                                m_dAgentObserved[cAgent].Add(gp);
                                m_dAgentHidden[cAgent].Remove(gp.Canonical());
                            }
                        }
        ...
        public PartiallySpecifiedState(PartiallySpecifiedState original)
        {
            ...
            if (SDRPlanner.MultiAgent)
            {
                m_dAgentObserved = new Dictionary<Constant, HashSet<Predicate>>();
                m_dAgentHidden = new Dictionary<Constant, HashSet<Predicate>>();
            }
            ...
            if (original.m_lObserved != null)
            {
                ...
                if(SDRPlanner.MultiAgent)
                {
                    foreach(Constant cAgent in original.m_dAgentObserved.Keys)
                    {
                        m_dAgentObserved[cAgent] = new HashSet<Predicate>(original.m_dAgentObserved[cAgent]);
                    }
                }
            }
            else
            ...
            if (original.m_lHidden != null)
            {
                ...
                if (SDRPlanner.MultiAgent)
                {
                    foreach (Constant cAgent in original.m_dAgentHidden.Keys)
                    {
                        m_dAgentHidden[cAgent] = new HashSet<Predicate>(original.m_dAgentHidden[cAgent]);
                    }
                }
            }
            else
        ...
        public PartiallySpecifiedState(BeliefState bs)
        {
            ...
            if (SDRPlanner.MultiAgent)
            {
                m_dAgentObserved = new Dictionary<Constant, HashSet<Predicate>>();
                m_dAgentHidden = new Dictionary<Constant, HashSet<Predicate>>();
                foreach (Constant cAgent in Problem.Domain.Agents)
                {
                    m_dAgentObserved[cAgent] = new HashSet<Predicate>();
                    m_dAgentHidden[cAgent] = new HashSet<Predicate>();
                }
                ...
                foreach (Predicate p in lCurrent)
                {
                    ...
                    if (SDRPlanner.MultiAgent)
                    {
                        foreach (Constant cAgent in Problem.Domain.Agents)
                        {
                            if (p.Negation)
                            {
                                if (!m_dAgentHidden[cAgent].Contains(p.Negate()))
                                    m_dAgentHidden[cAgent].Add(p.Negate());
                            }
                            else if (!m_dAgentHidden[cAgent].Contains(p))
                                m_dAgentHidden[cAgent].Add(p);
                        }
                    }
                }
            }
        ...
        public PartiallySpecifiedState(PartiallySpecifiedState sPredecessor, Action aGeneratingAction)
        {
            ...
            {
                m_dAgentObserved = new Dictionary<Constant, HashSet<Predicate>>();
                foreach (Constant cAgent in sPredecessor.m_dAgentObserved.Keys)
                {
                    m_dAgentObserved[cAgent] = new HashSet<Predicate>(sPredecessor.m_dAgentObserved[cAgent]);
                }
                m_dAgentHidden = new Dictionary<Constant, HashSet<Predicate>>();
                foreach (Constant cAgent in sPredecessor.m_dAgentHidden.Keys)
                {
                    m_dAgentHidden[cAgent] = new HashSet<Predicate>(sPredecessor.m_dAgentHidden[cAgent]);
                }
            }
            ...
            foreach (Predicate p in lPossibleEffects)
            {
                ...
                if(SDRPlanner.MultiAgent)
                {
                    if (!Problem.Domain.AlwaysKnown(p))
                    {
                        foreach (Constant cAgent in GeneratingAction.ExecutingAgents)
                        {
                            if (m_dAgentObserved[cAgent].Contains(p))
                                m_dAgentObserved[cAgent].Remove(p);
                            else if (m_dAgentObserved[cAgent].Contains(pNegate))
                                m_dAgentObserved[cAgent].Remove(pNegate);
                            m_dAgentHidden[cAgent].Add(p.Canonical());
                        }
                    }
                }
            }
        ...
        private bool AddToObservedList(Predicate p)
        {
            ...
            if(SDRPlanner.MultiAgent)
            {
                //if (p.Name.Contains("box-at"))
                //    Console.Write("*");
                //this is a hack - what we need to do is regress for each agent independently. We currently allow reasoning from one agent to enter the knowledge of another agent.
                foreach(Constant cAgent in GeneratingAction.ExecutingAgents)
                {
                    if (!Problem.Domain.AlwaysKnown(p))//another hack - focus only on the things that need observations and reasoning. For example, everybody knows the location of agents
                    {

                        m_dAgentObserved[cAgent].Remove(pNegate);
                        m_dAgentObserved[cAgent].Add(p);
                        m_dAgentHidden[cAgent].Remove(pCanonical);
                    }
                }
            }
        ...
        private bool IsApplicable(Action a)
        {
            ...
            if (false)// I don't think that this is needed
            {
                HashSet<Predicate> lAllObserved = new HashSet<Predicate>();

                foreach (Predicate p in m_lObserved)
                {
                    bool bAllObserved = true, bNoneOberved = true;
                    foreach (Constant cAgent in m_dAgentObserved.Keys)
                    {
                        if (m_dAgentObserved[cAgent].Contains(p) || m_dAgentObserved[cAgent].Contains(p.Negate()))
                            bNoneOberved = false;
                        else
                            bAllObserved = false;
                    }
                ...
                Formula fReduced = a.Preconditions.Reduce(lAllObserved);
                if (fReduced.IsTrue(lAllObserved))
                    return true;

                //this is a hack - we need here an agent based regression, but for simple problems this should work.
                return false;
                /*
                if (fReduced.IsFalse(lAllObserved))
                    return false;
            ...
            else
            {
                ...
                if (fReduced.IsTrue(m_lObserved, false))
                    return true;
                if (fReduced.IsFalse(m_lObserved, false))
                    return false;
                Formula fNegatePreconditions = fReduced.Negate();
                if (ConsistentWith(fNegatePreconditions, true))
                {
        ...
        public void ApplyOffline(Action a, out Formula fObserve, out PartiallySpecifiedState psTrueState, 
            out PartiallySpecifiedState psFalseState)
        {
            psTrueState = null;
            psFalseState = null;
            fObserve = null;
                        
            // Need to remove "false" when complex effects, e.g., cond, or, etc.
            if (a.Observe != null && a.Effects != null && false)
            {
                throw new NotImplementedException(); 
            }
            ...
            // For removing all generally known items from the computation.
            Action aTag = a.ApplyObserved(m_lObserved); 
            a = aTag;
            ...
            if (a.Observe != null)
            {
                if(a.Effects == null)
                {
                    ...
                    if (SDRPlanner.MultiAgent)
                    {
                        //need to check here whether the agent already observed
                    }
                    else
                    {
